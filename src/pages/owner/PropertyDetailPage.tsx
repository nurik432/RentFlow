...
import {
  ...
  ClipboardList, Edit, Trash2, User
} from 'lucide-react';

export default function PropertyDetailPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { properties, payments, utilityBills, meterReadings, tasks, deleteProperty, getTenantName } = useData();
  const { user } = useAuth();
          ...
          {propPayments.length > 0 ? (
            <div className="table-container" style={{ border: 'none' }}>
              <table className="table">
                <thead><tr><th>Месяц</th><th>Тип</th><th>Сумма</th><th>Статус</th><th>Дата оплаты</th></tr></thead>
                <tbody>
                  {propPayments.map(p => (
                    <tr key={p.id}>
      ...
      {activeTab === 'meters' && (
        <div className="card">
          {propReadings.length > 0 ? (
            <div className="table-container" style={{ border: 'none' }}>
              <table className="table">
                <thead><tr><th>Месяц</th><th>Холодн. вода</th><th>Горяч. вода</th><th>Электро (день)</th><th>Электро (ночь)</th><th>Газ</th></tr></thead>
                <tbody>
                  {propReadings.map(r => (
                    <tr key={r.id}>
